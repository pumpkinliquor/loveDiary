...
<script>
    ...
    $(document).ready(function(){
        ...
        //UI.makeTd('select','iclGubun','구매처/구매처').appendTo(rowWrap).addItem(codes.iclGubun);
        ...
        UI.makeTd('text','iclCompanyCode','구매처코드').appendTo(rowWrap);;
        UI.makeInput('hidden', 'V', {name: 'iclGubun', id: 'iclGubun'}).appendTo(rowWrap);
        UI.makeTd('text','iclCompanyName','구매처명').appendTo(rowWrap);
        ...
        try{
            ...
            if(Object.keys(objValue).length>0){
                $('.popupFlagTitle').show();
                lgUI.grid.popupGrid = lgUI.grid.makeGrid('popupGridSelectElement',colData,'/rest/client/clientList',0,objValue,{height:'auto',onSelectRow:function(){}});
            } else {
                $('.popupFlagTitle').hide();
            }
        } catch(e){
        ...
        lgUI.grid.popupGrid = lgUI.grid.makeGrid('popupGridElement',colData,'/rest/client/clientList',{},{'iclGubun':'V'},{onSelectRow:lgUI.event.gridPopupClick});
...
<div id="plusPopUpTabs" class="inner">
    <ul class="addTab">
        <li class="select"><a href="#tabs-grid" class="pageTitle">구매처 목록</a></li>
    </ul>
