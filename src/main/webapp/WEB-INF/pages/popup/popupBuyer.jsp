    ...
    $(document).ready(function(){
        ...
        lgUI.event.gridPopupClick=function(rowid){
            ...
            if(confirm('해당 정보를 선택하시겠습니까?')){
                ...
                // $('#icmCompanyCode option',wrapElement).prop('disabled', true);
                // $('#icmCompanyCode',wrapElement ).select2('destroy');
                // $('#icmCompanyCode',wrapElement ).select2({width:'100%'})
        ...
        //UI.makeTd('hidden','iclGubun','고객/구매처').appendTo(rowWrap).addItem(codes.iclGubun);
        ...
        UI.makeInput('hidden', 'V', {name: 'iclGubun', id: 'iclGubun'}).appendTo(rowWrap);
        UI.makeTd('text','iclName','구매처명').appendTo(rowWrap);
        ...
        colData.push({ label:'구매처코드',name:'iclCompanyCode',index:'ICL_COMPANY_CODE',classes:'celltc',width:'120px'});
        colData.push({ label:'구매처명',name:'iclCompanyName',index:'ICL_COMPANY_NAME',classes:'celltl',width:240});
        ...
        colData.push({ label:'소재지우편번호',name:'iclBusinessPostno',index:'ICL_BUSINESS_POSTNO',width:250,classes:'celltl',hidden:true});
        ...
        lgUI.grid.popupGrid = lgUI.grid.makeGrid('popupGridElement',colData,'/rest/sap/getClientList',{},{iclGubun:'V'},{onSelectRow:lgUI.event.gridPopupClick});
...
<div id="plusPopUpTabs" class="inner">
    <ul class="addTab">
        <li class="select"><a href="#tabs-grid" class="pageTitle">구매처 연동 목록</a></li>
    </ul>
