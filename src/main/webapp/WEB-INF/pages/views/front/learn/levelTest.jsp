...
<%-- 	<input type="hidden" name="acvId" value="<c:out value="${data.acvId}"/>" /> 학습 완료 체크할 때 사용 --%>
	...
	<input type="hidden" name="currentTestType" value="<c:out value="${empty data.currentTestType? 'level' :  data.currentTestType}"/>" /> <%-- 풀고있는 평가종류 / 틀렸을 경우 유사문제 출제를 위해 사용 --%>
</form>
<%-- 다음 출제문제 이동 Form --%>
...
<%-- 	<input type="hidden" name="acvId" value="<c:out value="${data.acvId}"/>" /> --%>
	...
	<input type="hidden" name="currentTestType" value="<c:out value="${empty data.currentTestType? 'level' :  data.currentTestType}"/>" /> <%-- 풀고있는 평가종류 / 틀렸을 경우 유사문제 출제를 위해 사용 --%>
	<input type="hidden" name="pagePath" value="levelTest" />
</form>
...
function goNext(){
	...
	if(isFinish){
		commonModalPopup('<h3>${data.levName} 학습이 완료되었습니다.</h2><br/><br/>Aigo 리포트에서 나의 레벨평가 결과와<br/>학습분석 내용을 확인하세요.');
		$popup.click({}, goReport);
	}else{
		goNextQuestion();
	}
}
function goNextQuestion(){
	$form.attr("action","/front/learn/levelTest").submit();
}
