...
<form id="answerform" name="answerform" method="post">
	...
	<input type="hidden" name="currentTestType" value="<c:out value="${empty data.currentTestType? 'weekly' :  data.currentTestType}"/>" /> <%-- 풀고있는 평가종류 / 틀렸을 경우 유사문제 출제를 위해 사용 --%>
...
function goNext(){
	...
	if(isFinish && $form.find("input[name='currentTestTypeSub']").val() == 'basic'){
		commonModalPopup('<h3>${data.acvName}</h2><br/>학습이 완료 되었습니다.<br/>Aigo 주간 리포트에서 나의 평가 결과와<br/>학습 분석 내용을 확인하세요.');
		$popup.click({}, goWeeklyReport);
