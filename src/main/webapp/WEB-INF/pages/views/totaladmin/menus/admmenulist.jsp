...
$(document).ready(function(){
    ...
    //페이지 래디 정의
        ...
        gridColumn.push({'data':'maSeq','title':'시퀀스','type':'','hidden':true});
        gridColumn.push({'data':'maParent','title':'메뉴루트','class':'tl'});
        gridColumn.push({'data':'maCode','title':'메뉴코드','class':'tl'});
        gridColumn.push({'data':'maLevel','title':'메뉴레벨',className:'hidden-xs-down'});
        // gridColumn.push({'data':'maModule','title':'메뉴명','class':'tl'});
        gridColumn.push({'data':'maName','title':'메뉴명','class':'tl'});
        gridColumn.push({'data':'maPath','title':'메뉴경로','class':'tl',className:'hidden-xs-down'});
        gridColumn.push({'data':'maSort','title':'정렬','class':'tl',hidden:true});
        gridColumn.push({'data':'maIsUse','title':'사용여부',className:'hidden-xs-down'});
        // gridColumn.push({'data':'maIcon','title':'메뉴아이콘','class':'tl'});
        gridColumn.push({'data':'regDate','title':'등록일','class':'tl','hidden':true});

        gridElement = plus.makeGrid('#gridElement',gridColumn,plus.makeAjax('/totaladmin/ajax/admmenus/menuList',{bbBbs:'BA'},'resultList'),{pageLength:50,attr:'속성'});
        ...
        $('#gridElement tbody').on('click','tr.even,tr.odd',function () {
            ...
            plus.makeInput('hidden','maSeq').prependTo(wrapElement);
            plus.makeInput('hidden','maParent').prependTo(wrapElement);
            plus.makeInput('hidden','delYn').prependTo(wrapElement);
            plus.makeInput('hidden','maLevel').prependTo(wrapElement);
            // plus.makeInput('hidden','maNavi').prependTo(wrapElement);
            plus.makeInput('hidden','regDate').prependTo(wrapElement);
            tr = plus.makeElement('tr','',{class:'box mt10  formDiv'}).appendTo(tableElement);
            tr = plus.makeElement('tr','',{class:'box mt10  formDiv'}).appendTo(tableElement);
            tr = plus.makeElement('tr','',{class:'box mt10  formDiv'}).appendTo(tableElement);
            plus.makeTd('text','maModule','모듈명').appendTo(tr);
            plus.makeTd('text','maName','메뉴명').appendTo(tr);
            tr = plus.makeElement('tr','',{class:'box mt10  formDiv'}).appendTo(tableElement);
            plus.makeTd('text','maCode','코드').appendTo(tr);
            plus.makeTd('text','maNavi','네비').appendTo(tr);
            tr = plus.makeElement('tr','',{class:'box mt10  formDiv'}).appendTo(tableElement);
            tr = plus.makeElement('tr','',{class:'box mt10  formDiv'}).appendTo(tableElement);
            plus.makeTd('text','maPath','메뉴경로').appendTo(tr);
            plus.makeTd('text','maSort','정렬').appendTo(tr);
            tr = plus.makeElement('tr','',{class:'box mt10  formDiv'}).appendTo(tableElement);
            plus.makeTd('select','maIsUse','사용여부').appendTo(tr).addItem({'y':'사용','n':'미사용'});
            plus.makeTd('text','maIcon','메뉴아이콘').appendTo(tr);
            tr = plus.makeElement('tr','',{class:'box mt10  formDiv'}).appendTo(tableElement);
