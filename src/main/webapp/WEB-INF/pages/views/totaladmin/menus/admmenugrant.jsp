...
$(document).ready(function(){
    $('.btnGrantAction').click(function(){
        ...
        $.each(gridElement.data(),function(i,v){
             ...
             putData.push({maSeq:rowData['maSeq'],'ugSeq':rowData['ugSeq'],mgGrant:$('.mg_grant').eq(i).val()});
	...
	$('#groupElement tbody').on('click','tr.even,tr.odd',function () {
        ...
        var gridColumn =[];
        gridColumn.push({'data':'maParent','title':'메뉴루트','type':'','hidden':true});
        gridColumn.push({'data':'maCode','title':'메뉴코드','type':'','hidden':true});

        gridColumn.push({'data':'maModule','title':'모듈명','type':''});
        gridColumn.push({'data':'maName','title':'메뉴명','type':''});
        ...
        gridColumn.push({'data':'maIsUse','title':'사용여부'});
        ...
        gridElement = plus.makeGrid('#gridElement',gridColumn,plus.makeAjax('/totaladmin/ajax/admmenus/menuGrant',{ugSeq:rowData['ugSeq']},'resultList'),{bPaginate:false,pageLength:100,attr:'속성',drawCallback:function(settings, json){
