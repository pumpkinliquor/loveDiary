...
$('#drag-and-drop-zone').dmUploader({ //
      ...
      onUploadSuccess: function(id, data){
        // A file was successfully uploaded
          $('#uploaderFile'+id).find('i').click(function() {
            if(confirm('해당 파일을 삭제하시겠습니까')){
              $('.media').has($(this)).remove();
            }
          });
        $('#uploaderFile'+id).find('.preview-img').attr('src','/common/imgView?filePath='+data.itafPath+'/'+data.itafFile+"&fileName="+data.itafOrFile)
        $('#uploaderFile'+id).append(UI.makeInput('hidden',JSON.stringify(data),{'class':'uploadData'}));


        ui_add_log('Server Response for file #' + id + ': ' + JSON.stringify(data));
        ui_add_log('Upload of file #' + id + ' COMPLETED', 'success');
        ui_multi_update_file_status(id, 'success', '업로드완료');
