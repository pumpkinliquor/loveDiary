...
(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    clickStartEditEventHandler:       function (event) {
	  ...
	  //item.mouse',_this.clickEndEditEventHandler.bind(this));
	  //mouseleave',_this.clickEndEditEventHandler.bind(this));
	  item.on('mouseleave',function(){
		 $(this).parent().find('.end-edit').click();
	  });
      ...
      spn.stop().hide(opt.slideAnimationDuration, function () {
        ...
        if (firstInput.get(0).nodeName !== 'SELECT'){
			...
			console.log(firstInput.next().val());
			$('#field').val(firstInput.next().val());
        }else {
			firstInput.val(firstInput.find('option:contains(' + spn.text() + ')').val());
			$('#field').val(firstInput.next().val());
			console.log(firstInput.next().val());
		}
        ...
        btnContainer.stop().show(opt.slideAnimationDuration, function () {
          // Show the edit panel
    ...
    createNewListItem:                function (data) {
	  ...
	  //blueprint.find('span').first().get(0).addEventListener('mouseleave',_this.clickEndEditEventHandler.bind(this));
